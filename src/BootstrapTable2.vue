...
<div class="bootstrap-table">
    <div class="fixed-table-toolbar">
        ...
        <div class="columns columns-{{options.buttonsAlign}} btn-group pull-{{options.buttonsAlign}}">
            ...
            <button v-if="options.showPaginationSwitch"
                class="btn btn-flat btn-{{options.buttonsClass}} btn-{{options.iconSize}}"
            ...
            <div v-if="options.showColumns" class="keep-open btn-group"
                ...
                <button type="button" data-toggle="dropdown"
                    class="btn btn-flat btn-flat btn-{{options.buttonsClass}} btn-{{options.iconSize}} dropdown-toggle">
    ...
    <div class="fixed-table-container"
        ...
        <div class="fixed-table-header" v-if="options.showHeader && options.height">
            <!--设置表格绝对高度时用到,这时候通过两个table来实现，tittle和body分别是不同的两个表格-->
        ...
        <div class="fixed-table-body">
            ...
            <table class="{{options.classes}}">
                <thead v-if="options.showHeader && !options.height">
                    <!--表格自适应高度时用到，如果是绝对高度，则thead是不会显示的，只显示下面的tbody-->
                    <tr v-for="_columns in columns">
                        <template v-for="column in _columns">
                            <th v-if="column.visible"
                                title="{{column.titleTooltip}}"
                ...
                <tbody>
                    <tr v-for="(i, item) in renderData"
                        ...
                        <template v-for="(j, field) in header.fields">
                            <template v-if="fieldColumns[j].visible">
                                <td v-if="fieldColumns[j].checkbox || fieldColumns[j].radio"
                                    ...
                                    checked="{{item[fieldColumns[j].field] ? 'true' : null }}"
                                ...
                                <td v-else
                                    ...
                                    v-on:dblclick="onTdClick(item, fieldColumns[j].field, $event)" v-bind:style="fieldColumns[j].style">
        ...
        <div v-if="options.pagination" class="fixed-table-pagination">
            <div class="pull-{{options.paginationDetailHAlign}} pagination-detail">
                <span class="pagination-info">
                    ...
                    <template v-else>
                    {{options.formatShowingRows(pageFromReal, pageToReal, options.totalRows)}}
                    </template>
                </span>
                <span v-if="!options.onlyInfoPagination" class="page-list">
                    ...
                    <span class="btn-group {{options.paginationVAlign == 'top' || options.paginationVAlign == 'both' ? 'dropdown' : 'dropup'}}">
                        <button type="button" class="btn btn-flat btn-{{options.buttonsClass}} btn-{{options.iconSize}} dropdown-toggle" data-toggle="dropdown">
...
var BootstrapTable = {
    ...
    methods: {
        ...
        initPagination: function () {
            ...
            this.pageFromReal = this.pageFrom;
            this.pageToReal = this.pageTo;
        },
        ...
        initServer: function (silent, query, url) {
            ...
            //如果选择maintainSelected模式，以前添加的会出现无效的情况，现在索性就全部删除，然后再重新添加一遍
            this.selected.items = [];
        ...
        trigger: function (name) {
            ...
            if(typeof this.triggerEvent[name] == 'function'){
                this.triggerEvent[name].apply(this,[args,this]);
            }
